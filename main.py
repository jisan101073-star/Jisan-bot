...
def setup_handlers(app, sessions_clients):
    ...
    async def handle_view_command(client, message):
        ...
        if len(args) < 2:
            await message.reply("❌ **Format:** `/view [Post_Link] [Amount]`")
            return
        ...
        try:
            ...
            await message.reply(f"✅ **Success!** Total `{success_count}` ta view add hoiche.")
...
async def start_bot():
    ...
    if not sessions:
        logger.warning("No SESSION_STRING found!")
        return
    ...
    logger.info("Main Pyrogram session started.")
    ...
    for session in sessions[1:]:
        try:
            ...
            logger.error(f"Extra session error: {e}")

    logger.info("All view bot sessions are running successfully!")
    await idle()
    
    for c in sessions_clients:
        await c.stop()
